...
public class AddCmsCategoryAction extends AddAction<CmsCategory> {
    ...
    public AddCmsCategoryAction(String operator, String siteCode, CmsCategoryVo vo) {
    ...
    protected void preprocess() {
        BusinessException.check(StringUtil.isNotBlank(vo.getCode()), "分类编码不能为空，分类创建失败！");
        BusinessException.check(StringUtil.isNotBlank(vo.getName()), "分类名称不能为空，分类创建失败！");
        BusinessException.check(StringUtil.isNotBlank(vo.getPath()), "分类访问路径不能为空，分类创建失败！");
        BusinessException.check(StringUtil.isNotBlank(vo.getStatus().getCode()), "分类状态不能为空，分类创建失败！");
        BusinessException.check(StringUtil.isNotBlank(vo.getTemplate().getCode()), "分类模板不能为空，分类创建失败！");
        BusinessException.check(vo.getPageSize() != 0, "分类每页内容数量必须大于零，分类创建失败！");

        CmsCategory category = categoryDao.findByUnsureCode(vo.getCode());
        BusinessException.check(category == null, "已存在编码为[{0}]的分类，分类创建失败！", vo.getCode());
    ...
    @Override
    protected CmsCategory createTargetEntity() {
        CmsCategory category = new CmsCategory(vo.getCode());
